...
class PairedCycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        A (source domain), B (target domain). A--- without makeup , B --- with makeup
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define discriminators
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_style.parameters(), self.netD_A.parameters(), self.netD_B.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def backward_D_style(self, real_pair, fake_pair):
        # Real
        real = torch.cat(real_pair, dim=1)
        pred_real = self.netD_style(real)
        loss_D_real = self.criterionGAN(pred_real, True)
        # Fake
        fake = torch.cat(fake_pair, dim=1)
        pred_fake = self.netD_style(fake.detach())
        loss_D_fake = self.criterionGAN(pred_fake, False)
        # Combined loss and calculate gradients
        loss_D = (loss_D_real + loss_D_fake) * 0.5
        loss_D.backward()
